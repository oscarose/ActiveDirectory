             ...
             steps{
                 git branch: 'master',
                     credentialsId: 'Github',
                         url: 'https://github.com/oscarose/ActiveDirectory.git'
             }
         }
                 ...
                 withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws', accessKeyVariable: 'aws_access_key', secretKeyVariable: 'aws_secret_key']]){
                     script {
                          if(aws_region=="us-east-1") {
                              ...
                              ansible-playbook  --extra-vars "ansible_python_interpreter=/usr/bin/python stack_name=${stack_name} state=${state} aws_region=${aws_region} aws_secret_key=$aws_secret_key aws_access_key=$aws_access_key" ${WORKSPACE}/ad.yaml
                              echo "stack creation is complete"
                              """
                          }
                          if(aws_region=="us-west-2") {
                              sh 'ansible-playbook --extra-vars "ansible_python_interpreter=/usr/bin/python stack_name=${stack_name} state=${state} aws_region=${aws_region} aws_secret_key=${aws_secret_key} aws_access_key=${aws_access_key}" ${WORKSPACE}/ad.yaml'
                          }
                     }
                 }
             }
